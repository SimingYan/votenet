...
def init_model_from_weights(
    ...
    for param_name in state_dict:
        param_prefix = param_name.split(".")[0]
        if "encoder" != param_prefix:
            continue

        if 'encoder.pointnet2_encoder' in param_name:
            newname = param_name.replace('encoder.pointnet2_encoder', 'backbone_net')
        else:
            newname = param_name.replace('encoder', 'backbone_net')
    ...
    for layername in all_layers.keys():
        ...
        if layername in state_dict:
            ...
            try:
                ...
                print(f"Wrong size layer:\t{layername}")
                not_init.append(layername)
                continue
